...
<div toplayer="1" class="wrap-box wrap-pb2 " id="<?php echo $funcid;?>" summaryid="TempletDetail" baseurl="<?php echo U('TempletDetail/index'); ?>">

   <div class="wrap-box-info ">
      <div class="wrap-title abe-ofl">
         <div class="tit abe-fl">组卷模板明细</div>
         <div class="abe-fr">
      ...
      <div class="table-box">
         <div class="table-in">
          ...
          <?php if ($search['status']==0): ?>
               <a href="javascript:void(0);"  class=" vi-blue vi-blue mlg_10 " onclick="return _asr.popupFun('<?php echo U("/Home/TempletDetail/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/TempletDetail/index?func=edit_base&id=$search[id]","");?>'); "> 编辑</a>
          <?php endif; ?>
          <?php endif; ?>
     ...
     <table border="0" cellspacing="0" cellpadding="0" class="pub-table-par">
        ...
        <tbody>
           ...
           <tr class= "even">
             ...
             <th>类型</th>
             <td><?php echo get_table_TempletDetail_type("$search[type]","name","") ; ?></td>
             <th>标题</th>
           ...
           <tr class= "odd">
             ...
             <th>要求类型</th>
             <td><?php echo get_table_TempletDetail_req_type("$search[req_type]","name","") ; ?></td>
             <th>知识点码</th>
             <td><?php echo $search["req_category_code"]; ?></td>
             <th>要求知识点</th>
             <td><?php echo $search["req_category_name"]; ?></td>
           </tr>
           <tr class= "even">
             <th>要求题型</th>
             <td><?php echo subcode_view('question:kind',$search['req_kind']) ; ?></td>
             <th>套题小题数</th>
             <td><?php echo $search["req_child_count"]; ?></td>
             <th>套题小题号</th>
             <td><?php echo get_table_TempletDetail_req_child_seq("$search[req_child_seq]","name","") ; ?></td>
             <th>抽取要求</th>
             <td><?php echo get_table_TempletDetail_extract("$search[extract]","name","") ; ?></td>
           </tr>
           <tr class= "odd">
             <th>创建时间</th>
       ...
       <div class="table-box">
         <div class="table-in">
            <form action="<?php echo U('TempletDetail/index?func=view'); ?>" id="<?php echo "$funcid"; ?>-Search" method="get">
...
搜索关闭状态，按需要打开 -->
              ...
              <?php if ($search['_tab']=='shijuanmingxi'): ?>
                <div class="table-box">
                  <div class="table-in">
                    <form id="<?php echo "$funcid"; ?>-Result"  action="<?php U('TempletDetail/op'); ?>" method="post">
                      <input type="hidden" name="<?php echo "$funcid"; ?>-last-url" id="<?php echo "$funcid"; ?>-last-url" value="<?php echo $__last_url; ?>" />
                      <input type="hidden" name="funcid" value="<?php echo "$funcid"; ?>" />
                      <input type="hidden" name="pfuncid" value="<?php echo "$pfuncid"; ?>" />
                      <input type="hidden" name="id" value="<?php echo $search["id"]; ?>" />
   ...
   <div class="data-oper" >
      ...
      <div class="data-oper-in " >
          ...
          <?php if (isset($rights['order_edit']) && $rights['order_edit']): ?>
               <input type="button" value="信息编辑" class="btn btn-blue mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/TempletDetail/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/TempletDetail/index?func=edit_base&id=$search[id]","");?>'); " />
          <?php endif; ?>
          ...
          <?php if (isset($rights['order_delete']) && $rights['order_delete']): ?>
               <input type="button" value="记录删除" class="btn btn-org mrg_10 " default-status="1" onclick="return _asr.confirm('确认操作', '请确认是否进行 信息删除 操作?', '', '<?php echo U("/Home/TempletDetail/index?func=delete&id=$search[id]") ; ?>','','<?php echo "$funcid"; ?>-Result'); " />
          <?php endif; ?>
