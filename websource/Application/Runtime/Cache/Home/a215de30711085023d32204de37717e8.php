...
<div toplayer="1" class="wrap-box wrap-pb2 " id="<?php echo $funcid;?>" summaryid="QuestionCategory" baseurl="<?php echo U('QuestionCategory/index'); ?>">

   <div class="wrap-box-info ">
      <div class="wrap-title abe-ofl">
         <div class="tit abe-fl">知识点分类</div>
         <div class="abe-fr">


          <?php if (isset($rights['import']) && $rights['import']): ?>
               <input type="button" value="数据导入" class="btn btn-blue mrg_10 " onclick="return _asr.popupFun('<?php echo U("/Home/QuestionCategory/index?func=import&") ; ?>', '<?php echo filterFuncId("/Home/QuestionCategory/index?func=import&","");?>'); " />
          <?php endif; ?>
      ...
      <div class="table-box">
         <div class="table-in">
          ...
          <?php if ($search['status']==0): ?>
               <a href="javascript:void(0);"  class=" vi-blue vi-blue mlg_10 " onclick="return _asr.popupFun('<?php echo U("/Home/QuestionCategory/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/QuestionCategory/index?func=edit_base&id=$search[id]","");?>'); "> 编辑</a>
          <?php endif; ?>
          <?php endif; ?>
     ...
     <table border="0" cellspacing="0" cellpadding="0" class="pub-table-par">
        ...
        <tbody>
           ...
           <tr class= "even">
             <th>上级</th>
             <td><?php echo $search["parent_id"]?str_replace("/".$search["name"]."/","", $search["full_path"]."/"):""; ?></td>
             <th>层级</th>
           ...
           <tr class= "even">
               ...
               <td></td>
             <th>状态</th>
             <td><?php echo get_table_QuestionCategory_status("$search[status]","name","") ; ?></td>
           </tr>
       ...
       <div class="table-box">
         <div class="table-in">
            <form action="<?php echo U('QuestionCategory/index?func=view'); ?>" id="<?php echo "$funcid"; ?>-Search" method="get">
...
搜索关闭状态，按需要打开 -->
              ...
              <?php if ($search['_tab']=='shijuanmingxi'): ?>
                <div class="table-box">
                  <div class="table-in">
                    <form id="<?php echo "$funcid"; ?>-Result"  action="<?php U('QuestionCategory/op'); ?>" method="post">
                      <input type="hidden" name="<?php echo "$funcid"; ?>-last-url" id="<?php echo "$funcid"; ?>-last-url" value="<?php echo $__last_url; ?>" />
                      <input type="hidden" name="funcid" value="<?php echo "$funcid"; ?>" />
                      <input type="hidden" name="pfuncid" value="<?php echo "$pfuncid"; ?>" />
                      <input type="hidden" name="id" value="<?php echo $search["id"]; ?>" />
          ...
          <?php if (isset($rights['master_view']) && $rights['master_view']): ?>
               <a href="javascript:void(0);"  class=" vi-blue vi-blue " onclick="return _asr.openLink('<?php echo U("/Home/QuestionCategory/index?func=view&id=$master[id]") ; ?>', '<?php echo filterFuncId(U("/Home/QuestionCategory/index?func=view&id=$master[id]"),"");?>' , '知识点分类' ,0); "><i class="iconfont abe-ft18">&#xe62e;</i></a>
          <?php endif; ?>
                        ...
                        <tbody>
                          ...
                          <?php if(is_array($list)): $i = 0; $__LIST__ = $list; if( count($__LIST__) == 0) : echo ""; else: foreach($__LIST__ as $key=>$master): $mod = ($i % 2 );++$i; ?>
                                  <tr <?php if($mod == "1"): ?>class="even"<?php endif; if($mod == "0"): ?>class="odd"<?php endif; ?>>
                                    ...
                                    <td><?php echo get_table_QuestionCategory_status("$master[status]","name","") ; ?></td>
                                    <td><?php echo system_format("DT", $master["create_time"],1) ; ?></td>
   ...
   <div class="data-oper" >
      ...
      <div class="data-oper-in " >
          ...
          <?php if ($search['status']==0): ?>
               <input type="button" value="信息编辑" class="btn btn-blue mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/QuestionCategory/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/QuestionCategory/index?func=edit_base&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']=='0'): ?>
               <input type="button" value="转有效" class="btn btn-blue mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/QuestionCategory/index?func=status_on&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/QuestionCategory/index?func=status_on&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']=='1'): ?>
               <input type="button" value="转无效" class="btn btn-org mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/QuestionCategory/index?func=status_off&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/QuestionCategory/index?func=status_off&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']==0): ?>
               <input type="button" value="记录删除" class="btn btn-org mrg_10 " default-status="1" onclick="return _asr.confirm('确认操作', '请确认是否进行 信息删除 操作?', '', '<?php echo U("/Home/QuestionCategory/index?func=delete&id=$search[id]") ; ?>','','<?php echo "$funcid"; ?>-Result'); " />
          <?php endif; ?>
          <?php endif; ?>
