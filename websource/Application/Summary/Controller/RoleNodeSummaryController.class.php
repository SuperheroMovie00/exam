...
class RoleNodeSummaryController extends BasicController {
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/RoleNodeSummary/index?func=search&").  "','".filterFuncId("RoleNodeSummary_Search","id=0")."','角色模块关系列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if($search["_showsearch"]=="hide"  ){
               ...
               $condition_role_node = join_condition($condition_role_node,"@role_node.node_name",$search["node_name"],"char");
               ...
               $condition_role_node = join_condition($condition_role_node,"@role_node.module",$search["module"],"char");
           }
