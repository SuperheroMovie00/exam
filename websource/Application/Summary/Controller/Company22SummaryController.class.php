...
//
//注释: Company22Summary - style22列表
//
use Home\Controller\BasicController;
use Think\Log;
class Company22SummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'Company22Summary', '/Home/user', '/Home/Company', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"Company22Summary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"Company22Summary","Action"=>"search") ,
                         array("key"=>"export","func"=>"Company22Summary","Action"=>"export") ,
                         array("key"=>"master_check","func"=>"/Home/user","Action"=>"unlock") ,
                         array("key"=>"detailarea1","func"=>"Company22Summary","Action"=>"detailarea1") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"Company22Summary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "Company22Summary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/Company22Summary/index?func=search&").  "','".filterFuncId("Company22Summary_Search","id=0")."','style22列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("Company22Summary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("Company22Summary-PageSize", $page_size);
        ...
        $html = $this->fetch("Company22Summary:index");
        echo $html;
    }
    ...
    public function detailarea1($data) {
        ...
        $html = $this->fetch("Company22Summary:detailarea1");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("Company22Summary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("Company22Summary", 'gbk', 'utf-8');
