...
class RoleUserSummaryController extends BasicController {
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/RoleUserSummary/index?func=search&").  "','".filterFuncId("RoleUserSummary_Search","id=0")."','角色用户关系列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           $join="";
           $count_sql = "select count(*) as cnt from @role_user  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           ...
           $sql = "select #selectfields# from @role_user  #join# Where 1=1 #condition# #orderby#";
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @role_user  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_role_user,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
