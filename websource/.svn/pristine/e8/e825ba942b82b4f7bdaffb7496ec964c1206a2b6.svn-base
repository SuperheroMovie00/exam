...
//
//注释: GenNumberSummary - 系统序号列表
//
use Home\Controller\BasicController;
use Think\Log;
class GenNumberSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'GenNumberSummary', '/Home/%table%', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"GenNumberSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"GenNumberSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"GenNumberSummary","Action"=>"export") ,
                         array("key"=>"status","func"=>"/Home/%table%","Action"=>"%action%")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"GenNumberSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "GenNumberSummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/GenNumberSummary/index?func=search&").  "','".filterFuncId("GenNumberSummary_Search","id=0")."','系统序号列表', 1",""));
    ...
    private function search($data) {
       ...
       $condition="";
       $condition_gen_number="";
       if($bsearch) {
           //关键字条件
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_gen_number = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_gen_number = join_condition($condition_gen_number,"@gen_number.type",$search["type"],"char");
               $condition_gen_number = join_condition($condition_gen_number,"@gen_number.mainkey",$search["mainkey"],"char");
               $condition_gen_number = join_condition($condition_gen_number,"@gen_number.subkey",$search["subkey"],"char");
               $condition_gen_number = join_condition2($condition_gen_number,"@gen_number.seqno",$search["seqno"],$search["seqno2"],"int");
               $condition_gen_number = join_condition2($condition_gen_number,"@gen_number.create_time",$search["create_time"],$search["create_time2"],"datetime");
           }

           //增加 tab 条件
           $condition_gen_number = $this->tabsheet_condition($condition_gen_number ,$search["_tab"]);
           //select fields
           $selectfields=" @gen_number.id ";
           $selectfields.=", @gen_number.type ";
           $selectfields.=", @gen_number.mainkey ";
           $selectfields.=", @gen_number.subkey ";
           $selectfields.=", @gen_number.seqno ";
           $selectfields.=", @gen_number.create_time ";
           $selectfields.=", @gen_number.modify_time ";


           $page_size = I("request.pagesize/d");
           if ($page_size<=0){
               $page_size = session("GenNumberSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("GenNumberSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @gen_number  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_gen_number,$count_sql);
           ...
           $sql = "select #selectfields# from @gen_number  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_gen_number,$sql);
        ...
        $html = $this->fetch("GenNumberSummary:index");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("GenNumberSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @gen_number  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_gen_number,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        $str = mb_convert_encoding("GenNumberSummary", 'gbk', 'utf-8');
