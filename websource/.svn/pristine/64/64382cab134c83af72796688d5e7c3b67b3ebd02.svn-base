...
//    }
    ...
    static public function getAccessList($authId) {
        ...
        $db     =   Db::getInstance(C('RBAC_DB_DSN'));
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
        $sql    =   "select node.id,node.name,node.module from ".
            $table['role']." as role,".
            $table['user']." as user,".
            $table['access']." as access ,".
            $table['node']." as node ".
        ...
        foreach ($apps as $v) {
            if(trim($v['module'])!=''){
                $module=str_replace("index?func=","",$v['module']);
                $module=explode("/",strtoupper(trim($module,'/')));
                $access[$module[0]][$module[1]][$module[2]]=true;
            }
        }
