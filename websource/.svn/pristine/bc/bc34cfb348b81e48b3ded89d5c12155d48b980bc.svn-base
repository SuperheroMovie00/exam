...
class QuestionCategorySummaryController extends BasicController {
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if($search["_showsearch"]=="hide"  ){
               ...
               $condition_question_category = join_condition($condition_question_category,"@question_category.parent_id",$search["parent_id"],"int");
           ...
           $selectfields.=", @question_category.parent_id ";
           ...
           $orderby = $this->get_orderby("sort",$search["_tab"]);
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        foreach($list as $master) {
            ...
            if ($show_list['parent_id']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_QuestionCategory_byID("$master[parent_id]","name"))."\t,";
            }
            if ($show_list['code']==1  || empty($show_list )) {
