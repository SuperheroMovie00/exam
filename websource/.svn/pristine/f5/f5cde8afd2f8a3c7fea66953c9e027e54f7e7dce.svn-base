...
//
//注释: CompanyUserSummary - 公司用户列表
//
use Home\Controller\BasicController;
use Think\Log;
class CompanyUserSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'CompanyUserSummary', '/Home/%table%', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"CompanyUserSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"CompanyUserSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"CompanyUserSummary","Action"=>"export") ,
                         array("key"=>"status","func"=>"/Home/%table%","Action"=>"%action%")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"CompanyUserSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "CompanyUserSummary";
    ...
    private function columnsetting($data){
        ...
        $data['summary']='CompanyUserSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/CompanyUserSummary/index?func=search&").  "','".filterFuncId("CompanyUserSummary_Search","id=0")."','公司用户列表', 1",""));
    ...
    private function search($data) {
       ...
       //读取页面参数
       $search["company_id"] = I("request.company_id");
       $search["department_id"] = I("request.department_id");
       ...
       $condition="";
       $condition_company_user="";
       if($bsearch) {
           //关键字条件
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_company_user = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_company_user = join_condition($condition_company_user,"@company_user.company_id",$search["company_id"],"int");
               $condition_company_user = join_condition($condition_company_user,"@company_user.department_id",$search["department_id"],"int");
               $condition_company_user = join_condition($condition_company_user,"@company_user.user_id",$search["user_id"],"int");
               $condition_company_user = join_condition($condition_company_user,"@company_user.level",$search["level"],"int");
           }

           //增加 tab 条件
           $condition_company_user = $this->tabsheet_condition($condition_company_user ,$search["_tab"]);
           //select fields
           ...
           if ($page_size<=0){
               $page_size = session("CompanyUserSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("CompanyUserSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @company_user  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_company_user,$count_sql);
           ...
           $sql = str_replace("#condition#",$condition_company_user,$sql);
        ...
        $html = $this->fetch("CompanyUserSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        $str_header = "";
        if ($show_list['company_id']==1 || empty($show_list)){
            $str_header .= "公司,";
        }
        if ($show_list['department_id']==1 || empty($show_list)){
            $str_header .= "部门,";
        }
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("CompanyUserSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @company_user  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_company_user,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        foreach($list as $master) {
            $str_line="";
            if ($show_list['company_id']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_Company_byID("$master[company_id]","name"))."\t,";
            }
            if ($show_list['department_id']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_Department_byID("$master[department_id]","name"))."\t,";
            }
            if ($show_list['user_id']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_User_byID("$master[user_id]","name"))."\t,";
            }
            if ($show_list['level']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_CompanyUser_level("$master[level]","name"))."\t,";
            }
        ...
        $str = mb_convert_encoding("CompanyUserSummary", 'gbk', 'utf-8');
