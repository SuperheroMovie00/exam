...
//
//注释: Company21Summary - style21列表
//
use Home\Controller\BasicController;
use Think\Log;
class Company21SummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'Company21Summary', '/Home/user', '/Home/Company', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"Company21Summary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"Company21Summary","Action"=>"search") ,
                         array("key"=>"export","func"=>"Company21Summary","Action"=>"export") ,
                         array("key"=>"master_check","func"=>"/Home/user","Action"=>"unlock") ,
                         array("key"=>"detailarea1","func"=>"Company21Summary","Action"=>"detailarea1") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"Company21Summary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "Company21Summary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/Company21Summary/index?func=search&").  "','".filterFuncId("Company21Summary_Search","id=0")."','style21列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("Company21Summary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("Company21Summary-PageSize", $page_size);
        ...
        $html = $this->fetch("Company21Summary:index");
        echo $html;
    }
    ...
    public function detailarea1($data) {
        ...
        $html = $this->fetch("Company21Summary:detailarea1");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("Company21Summary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("Company21Summary", 'gbk', 'utf-8');
