...
//
//注释: Company10Summary - style10列表
//
use Home\Controller\BasicController;
use Think\Log;
class Company10SummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'Company10Summary', '/Home/user', '/Home/Company', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"Company10Summary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"Company10Summary","Action"=>"search") ,
                         array("key"=>"export","func"=>"Company10Summary","Action"=>"export") ,
                         array("key"=>"master_check","func"=>"/Home/user","Action"=>"unlock") ,
                         array("key"=>"detailarea1","func"=>"Company10Summary","Action"=>"detailarea1") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"Company10Summary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "Company10Summary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/Company10Summary/index?func=search&").  "','".filterFuncId("Company10Summary_Search","id=0")."','style10列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("Company10Summary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("Company10Summary-PageSize", $page_size);
        ...
        $html = $this->fetch("Company10Summary:index");
        echo $html;
    }
    ...
    public function detailarea1($data) {
        ...
        $html = $this->fetch("Company10Summary:detailarea1");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("Company10Summary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("Company10Summary", 'gbk', 'utf-8');
