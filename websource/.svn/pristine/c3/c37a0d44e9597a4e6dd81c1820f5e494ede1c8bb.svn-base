...
class QimenClient
{
	...
	public $gatewayUrl = "http://qimenapi.tbsandbox.com/router/qimen/service";
	//public $gatewayUrl = "http://qimen.api.taobao.com/router/qimen/service";
	...
	protected function generateSign($params, $body)
	{
		ksort($params);
		...
		unset($k, $v);
		$stringToBeSigned .= $body.$this->secretKey;
	...
	public function execute($request, $session = null,$bestUrl = null)
	{
		...
		$body = $this->createXmlBody($apiParams);
		//签名
		$sysParams["sign"] = $this->generateSign($sysParams, $body);
		...
		try
		{
			if(count($fileFields) > 0){
				...
				$resp = $this->curl($requestUrl, $body);
			}
		}
		catch (Exception $e)
		{
	...
	private function createXmlBody($arr) {
		$xml = "<request>";
