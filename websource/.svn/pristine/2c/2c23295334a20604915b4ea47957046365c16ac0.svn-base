...
//
//注释: CompanySummary - 公司信息列表
//
use Home\Controller\BasicController;
use Think\Log;
class CompanySummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/Company', 'CompanySummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/Company","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"CompanySummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"CompanySummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"CompanySummary","Action"=>"export") ,
                         array("key"=>"master_view","func"=>"/Home/Company","Action"=>"view") ,
                         array("key"=>"master_edit","func"=>"/Home/Company","Action"=>"edit") ,
                         array("key"=>"master_delete","func"=>"/Home/Company","Action"=>"delete") ,
                         array("key"=>"detail","func"=>"CompanySummary","Action"=>"detail") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"CompanySummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "CompanySummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/CompanySummary/index?func=search&").  "','".filterFuncId("CompanySummary_Search","id=0")."','公司信息列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("CompanySummary-PageSize");
               if (!$page_size) {
                    $page_size = 1000;    //define on summary xml
               }
           }
           session("CompanySummary-PageSize", $page_size);
        ...
        $html = $this->fetch("CompanySummary:index");
        echo $html;
    }
    ...
    public function detail($data) {
        ...
        $html = $this->fetch("CompanySummary:detailindex");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("CompanySummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("CompanySummary", 'gbk', 'utf-8');
