    ...
    static public function dispatch() {
        ...
        if('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') ||  !Route::check()) ){   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            if(C('URL_PARAMS_BIND') && 1 == C('URL_PARAMS_BIND_TYPE')){
                ...
                preg_replace_callback('/(\w+(\[([0-9])*\])*)\/([^\/]+)/', function($match) use(&$var){
                	if($match[2] != "") {
                		$m = str_replace($match[2], "", $match[1]);
                		$k = str_replace(array("[", "]"), "", $match[2]);
                		if($k != "") {
                			$var[$m][$k] = strip_tags($match[4]);
                		} else {
                			$var[$m][] = strip_tags($match[4]);
                		}
                	} else {
                		$var[$match[1]]=strip_tags($match[4]);
                	}
                }, implode('/',$paths));
            }
